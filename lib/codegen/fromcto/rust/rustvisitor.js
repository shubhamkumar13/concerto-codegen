...
class RustVisitor {
    ...
    visitModelManager(modelManager, parameters) {
        ...
        // adjacency list to create an entry for lib.rs
        const tempMap = createAdjList();

        for (const namespace of modelManager.getNamespaces()) {
            const namespaceFileName = this.toValidRustName(namespace);
            tempMap.get("export").push(`pub mod ${namespaceFileName};`);
        }

        // Insert adjacency list
        rustAst.set(fileName, tempMap);
    ...
    visitModelFile(modelFile, parameters) {
        ...
        tempMap.get("import").push(`use serde::{ Deserialize, Serialize }`);
        ...
        modelFile.getImports().map(importString =>
            ...
            .forEach(namespace => {
                tempMap.get("import").push(`use crate::${this.toValidRustName(namespace)}::*;`);
            });
        ...
        rustAst.set(`${fileName}.rs`, tempMap);
        return null;
    }
    ...
    visitAssetDeclaration(assetDeclaration, parameters) {
        ...
        parameters.fileWriter.writeLine(0, 'assetDeclaration - ' + assetDeclaration.getName());
        return null;
    }
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        parameters.fileWriter.writeLine(0, 'classDeclaration - ' + classDeclaration.getName());
        return null;
    }
    ...
    visitField(field, parameters) {
        console.log('entering visitField', field.getName());
        parameters.fileWriter.writeLine(0, 'field - ' + field.getName());
        return null;
    }
    ...
    visitEnumValueDeclaration(enumValueDeclaration, parameters) {
        ...
        parameters.fileWriter.writeLine(0, 'enumValueDeclaration - ' + enumValueDeclaration.getName());
        return null;
    }
    ...
    visitRelationshipDeclaration(relationshipDeclaration, parameters) {
        ...
        parameters.fileWriter.writeLine(0, 'relationshipDeclaration - ' + relationshipDeclaration.getName());
        return null;
    }
}
