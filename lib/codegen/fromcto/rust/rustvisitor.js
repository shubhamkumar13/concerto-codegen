...
class RustVisitor {
    ...
    visitField(field, parameters) {
        ...
        if (field.isOptional?.() || this.isDateField(field.type)) {
    ...
    addUtilsModelFile(parameters) {
        ...
        parameters.fileWriter.writeLine(1, '');
        parameters.fileWriter.writeLine(0, '#[derive(Debug, Serialize, Deserialize)]');
        parameters.fileWriter.writeLine(0, 'struct Helper(');
        parameters.fileWriter.writeLine(1, '#[serde(with = "bson::serde_helpers::chrono_datetime_as_bson_datetime")]');
        parameters.fileWriter.writeLine(1, 'chrono::DateTime<Utc>,');
        parameters.fileWriter.writeLine(0, ' );');
        parameters.fileWriter.writeLine(1, '');
        parameters.fileWriter.writeLine(0, 'pub fn serialize_datetime<S>(datetime: &Option<chrono::DateTime<Utc>>, serializer: S) -> Result<S::Ok, S::Error>')
        parameters.fileWriter.writeLine(0, 'where')
        parameters.fileWriter.writeLine(1, 'S: Serializer,')
        parameters.fileWriter.writeLine(0, '{')
        parameters.fileWriter.writeLine(1, 'datetime.map(Helper).serialize(serializer)')
        parameters.fileWriter.writeLine(0, '}')
        parameters.fileWriter.writeLine(0, '')
        parameters.fileWriter.writeLine(0, "pub fn deserialize_datetime<'de, D>(deserializer: D) -> Result<Option<chrono::DateTime<Utc>>, D::Error>")
        parameters.fileWriter.writeLine(0, 'where')
        parameters.fileWriter.writeLine(1, "D: Deserializer<'de>,")
        parameters.fileWriter.writeLine(0, '{')
        parameters.fileWriter.writeLine(1, 'let helper: Option<Helper> = Option::deserialize(deserializer)?;')
        parameters.fileWriter.writeLine(1, ' Ok(helper.map(|Helper(i)| i))')
        parameters.fileWriter.writeLine(0, '}')
        parameters.fileWriter.closeFile()
    }
}
